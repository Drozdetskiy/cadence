...
- [ ] Create `src/rlx/plan/__init__.py` with re-exports
- [ ] Create `src/rlx/plan/parse.py` with types and parsing:
...
- [ ] Create `src/rlx/plan/plan.py` with selection and branch name extraction:
...
- [ ] Write tests in `tests/test_plan.py` covering: parse_plan with various markdown structures, TaskStatus determination, checkbox actionability, extract_branch_name with date prefixes, file_has_uncompleted_checkbox, Selector with tmp_path fixtures
...
- [ ] Create `src/rlx/git/backend.py` with `DiffStats` dataclass and `ExternalBackend` class:
...
- [ ] Create `src/rlx/git/service.py` with `Service` class:
...
- [ ] Create `src/rlx/git/__init__.py` that re-exports: `GitChecker`, `is_git_repo`, `get_default_branch`, `head_hash` (preserve backward compat), plus new exports `Service`, `DiffStats`
...
- [ ] Delete old `src/rlx/git.py`
- [ ] Update imports in `src/rlx/cli.py` and any other files that import from `rlx.git` -- existing imports should continue to work via `__init__.py` re-exports
- [ ] Update `tests/test_git.py` imports if needed
- [ ] Write tests in `tests/test_git_service.py` covering: ExternalBackend operations with tmp_path git repos, Service.create_branch_for_plan flow, Service.move_plan_to_completed, DiffStats parsing, commit trailer appending, _resolve_filesystem_case, _extract_path_from_porcelain
...
- [ ] Create `src/rlx/defaults/prompts/task.txt` with the full task prompt from the spec (the exact text is in `docs/reference/08-prompts.md`). The prompt uses `{{PLAN_FILE}}`, `{{PROGRESS_FILE}}`, `{{GOAL}}`, `{{DEFAULT_BRANCH}}` variables and `<<<RLX:ALL_TASKS_DONE>>>` / `<<<RLX:TASK_FAILED>>>` signals
- [ ] Add `build_task_prompt()` to `src/rlx/processor/prompts.py`:
...
- [ ] Write tests covering: build_task_prompt produces expected output with variables substituted, commit trailer appended when configured, goal format matches spec
...
- [ ] Add `run_tasks_only()` to Runner -- sets PhaseTask, calls `run_task_phase()`, requires plan_file
- [ ] Add `run_full()` to Runner -- in v0.2 this only calls `run_task_phase()` (review/finalize phases are v0.3). Sets PhaseTask, validates plan_file not empty
- [ ] Update `run()` dispatch to handle `"full"` and `"tasks-only"` modes in addition to `"plan"`
- [ ] Implement `run_task_phase()`:
...
- [ ] Implement `has_uncompleted_tasks() -> bool`:
...
- [ ] Implement `sleep_with_cancel(duration)` using threading.Event.wait(timeout)
- [ ] Add break/pause fields to Runner: `break_event: threading.Event | None`, `pause_handler: Callable[[], bool] | None`
- [ ] Add setter methods: `set_break_event(event)`, `set_pause_handler(fn)`
- [ ] Add helpers: `_is_break() -> bool` (break_event.is_set() and not cancelled), `_clear_break()`
- [ ] Add `last_session_timed_out: bool` field, `session_timeout: float` from config
- [ ] Add `resolve_plan_file_path() -> str` -- check original path, then completed/ subdir
- [ ] Update RunnerConfig if needed (session_timeout field)
- [ ] Update Logger protocol to include `print_aligned` method (used by task phase output)
- [ ] Write tests covering: run_task_phase with mock executor (COMPLETED signal, FAILED with retry, max iterations), has_uncompleted_tasks with various plan states, next_plan_task_position, break/pause flow, session timeout, sleep_with_cancel
...
- [ ] Add `ask_yes_no(prompt) -> bool` to `src/rlx/input.py` -- prompt with `[y/N]`, "y"/"yes" -> True, everything else -> False, EOF -> False
- [ ] Implement `run_task_mode(task_file: Path)` in `src/rlx/cli.py`:
...
- [ ] Add SIGQUIT handler for break/pause:
...
- [ ] Add `display_stats(stats: DiffStats, elapsed: str, branch: str)` helper
- [ ] Wire `--task` flag in main(): replace "not implemented" error with `run_task_mode(task)` call
- [ ] Write tests covering: run_task_mode with mocked git.Service and Runner, ask_yes_no various inputs, determine_mode with --task flag, display_stats formatting
...
- [ ] Verify all new modules have test coverage

### Task 7: Update documentation

- [ ] Update `CLAUDE.md` with new package structure (plan/, git/ package instead of git.py)
