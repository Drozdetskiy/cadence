# Справочник проекта cadence

Эта директория содержит описание архитектуры cadence -- Python-порта ralphex с упрощенным дизайном. Каждый документ описывает конкретный слой системы с детализацией, достаточной для реализации.
...
2. [Система конфигурации](02-config.md) - все поля конфига с типами и дефолтами, каскад загрузки (defaults in code -> `.cadence/config.toml` -> CLI flags), merge-стратегия, шаблонные переменные, frontmatter агентов
...
Ниже собраны ключевые Go-паттерны, используемые в ralphex, и их Python-эквиваленты для cadence.
...
- Каскад: defaults в коде -> `.cadence/config.toml` -> CLI flags
...
## Рекомендуемый стек для cadence
