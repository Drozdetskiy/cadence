...
Справочный документ по всем агентам ревью для Python-порта cadence.

## Обзор системы агентов

Агенты -- это специализированные промпты для ревью, запускаемые как субагенты Claude Code через Task tool. Хранятся в пакете cadence как системные defaults.

### Расположение и приоритет

Два уровня загрузки:

- Системный: встроен в пакет cadence (defaults)
- Локальный: `.cadence/agents/<name>.txt`
...
1. Загрузка из файлов с 2-level fallback (local `.cadence/agents/` -> системный в пакете)
