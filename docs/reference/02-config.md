...
```
CLI flags  >  local config (.cadence/config.toml)  >  defaults (in code)
```

Глобальный уровень (`~/.config/`) отсутствует. Конфигурация привязана к конкретному проекту.

Основные функции:
- `config.load(config_dir)` -- обнаруживает `.cadence/`, загружает config.toml, мержит с defaults
- `config.detect_local_dir()` -- ищет `.cadence/` в cwd

### Порядок загрузки

1. Загрузка defaults из кода (dict/dataclass)
2. Если `.cadence/config.toml` существует -- парсинг через `tomllib` (stdlib, Python 3.11+)
...
YAML overrides via `--config <path>` (or auto-discovered `cadence-config.yaml` next to the plan/task file) override these per-mode TOML defaults at load time.
