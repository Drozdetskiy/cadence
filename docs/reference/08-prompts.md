...
Справочный документ по всем промпт-файлам для Python-порта cadence.

## Обзор системы промптов

Промпты хранятся в пакете cadence как embedded-ресурсы и могут быть переопределены пользователем:
- Локально (проект): `.cadence/prompts/<name>.txt`
...
- `<<<CADENCE:ALL_TASKS_DONE>>>` -- все задачи завершены (COMPLETED mapped)
- `<<<CADENCE:TASK_FAILED>>>` -- задача провалена (FAILED mapped)
...
If NO Task section has [ ] but ## Success criteria, ## Overview, or ## Context still has [ ]: either satisfy those items and mark them [x] if actionable, or output <<<CADENCE:ALL_TASKS_DONE>>> if they are verification-only (manual testing, deployment, etc.) — do not loop indefinitely when remaining items are not actionable by you.
...
- If NO more [ ] checkboxes in the entire plan, output exactly: <<<CADENCE:ALL_TASKS_DONE>>>
- If more Task sections have [ ] checkboxes, STOP HERE - do not continue

If any phase fails after reasonable fix attempts, output exactly: <<<CADENCE:TASK_FAILED>>>
...
- `<<<CADENCE:QUESTION>>>` + JSON `{"question": "...", "options": [...]}` + `<<<CADENCE:END>>>` -- вопрос пользователю
- `<<<CADENCE:PLAN_DRAFT>>>` + содержимое плана + `<<<CADENCE:END>>>` -- черновик для ревью
- `<<<CADENCE:PLAN_READY>>>` -- план записан на диск
- `<<<CADENCE:TASK_FAILED>>>` -- пользователь отклонил план
...
- Output exactly: <<<CADENCE:TASK_FAILED>>>
...
- `<<<CADENCE:REVIEW_DONE>>>` -- ревью завершено, проблем не найдено
- `<<<CADENCE:TASK_FAILED>>>` -- найдены проблемы, не удалось исправить
...
- `<<<CADENCE:REVIEW_DONE>>>` -- ревью завершено, проблем не найдено
- `<<<CADENCE:TASK_FAILED>>>` -- проблемы не удалось исправить
