...
| `claude_args` | string | `"--dangerously-skip-permissions --verbose"` | Аргументы для claude |
...
| `review_model` | string | `"claude-opus-4-7"` | Модель для review фаз |
...
|----------|-----------|----------|
| `replace_base_variables()` | PLAN_FILE, PROGRESS_FILE, GOAL, DEFAULT_BRANCH | Базовый набор для всех промптов |
