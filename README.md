...
Tasks live in their own subdirectory under `cdc-tasks/<NNNN-slug>/` (configurable via `tasks_root`). The free-form description goes into a file named `init`; the generated plan is written next to it as `plan` (the `init` → `plan` mapping is built into `--plan`, and the prompt-file name is configurable via `init_prompt_name`). A per-task `config.yaml` next to the prompt is auto-discovered.
